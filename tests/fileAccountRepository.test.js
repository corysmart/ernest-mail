...
describe('FileAccountRepository', () => {
    ...
    describe('create', () => {
        ...
        it('throws when account already exists for email', async () => {
            ...
            await expect(repo.create({ email: 'dup@example.com', provider: 'ses' })).rejects.toThrow('Account already exists for email dup@example.com');
        });
    ...
    describe('concurrent writes (write lock)', () => {
        it('serializes concurrent creates', async () => {
            const promises = Array.from({ length: 5 }, (_, i) => repo.create({
